...
describe('Diamond proxy tests', function () {
    ...
    it('should freeze the diamond storage', async () => {
        ...
        const diamondCutFacetProposeCalldata = diamondCutFacet.interface.encodeFunctionData('proposeDiamondCut', [
            [],
            diamondProxyTest.address
        ]);
        await proxy.fallback({ data: diamondCutFacetProposeCalldata });
        const diamondCutFacetExecuteCalldata = diamondCutFacet.interface.encodeFunctionData(
